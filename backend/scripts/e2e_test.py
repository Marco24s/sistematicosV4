...
def run_e2e():
    ...
    admin_user = db.query(SystemUser).filter_by(username="admin").first()
    token = create_jwt_token({"sub": str(admin_user.id), "roles": ["ADMIN"]})
    ...
    events = db.query(StoredDomainEvent).filter(StoredDomainEvent.aggregate_id == pump.id).all()
    ...
    src = db.query(ServiceRelease).filter_by(asset_id=pump.id).first()
    if src:
